...
public:
    ...
    void step(); // Goes through one full move and breed action for all the bugs.
    void resetStepCount() { stepCount = 0; } // Resets the stepCount variable to 0
    ...
    int getBugType(int x, int y); // returns an integer based on the type of bug at the selected box. 2 = doodlebug, 1 = ant, 0 = empty
    int getStepCount() const { return stepCount; } // returns the number of times step() has been called since the last resetStepCount() call
private:
